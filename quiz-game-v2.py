...
# En esta versión tiene algunas mejoras:
    ...
    # respuestas y cargar la puntuación final (v1)

    # Se utiliza la función .lower() porque las respuestas las deja en minusculas
    # independientemente de como se ha escrito la respuesta (v1)
    
    # Se añade la clasificacion de la puntuacion obtenida en el juego (APROBADO, 
    # SUSPENDIDO) (v2)
...
if (int(respuesta[4]) == "22"):
    puntuacion += 1
elif (int(respuesta[4]) != "22"):
    puntuacion = puntuacion

if (puntuacion >= len(array_preguntas)):
    clasifiacion = "APROBADO"
elif (puntuacion < len(array_preguntas)):
    clasifiacion = "SUSPENDIDO"

print("LA PUNTUACIÓN ES DE {} PUNTOS Y HAS {} EL TEST".format(puntuacion, clasifiacion))
